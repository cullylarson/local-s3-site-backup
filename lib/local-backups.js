...
const makeDatabaseBackup = (today, user, pass, name, host, port, fileNameFormat, dest) => {
    ...
    return pipe([
        spawn('mysqldump', ['-h', host, '-u', user, '-P', port, name], { env: { MYSQL_PWD: pass } }),
        spawn('gzip', ['-']),
    ], destStream)
        .catch(err => {
...
const makeFilesBackup = (today, sourceFolder, fileNameFormat, destFolder) => {
    ...
    return pipe([
        spawn('tar', ['--warning=no-file-changed', '--ignore-failed-read', '-cf', '-', sourceFolderRelative], { cwd: sourceParentFolder }), // sourceFolderRelative and cwd so that the tar doesn't contain the full path to the source folder
        spawn('gzip', ['-']),
    ], destStream)
        .catch(err => {
            // try twice to remove the dest file, since it likely doesn't have anything useful in it
