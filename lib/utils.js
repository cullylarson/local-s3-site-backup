...
const promoteLocalBackups = R.curry((num, weeklyDest, monthlyDest, fileInfos) => {
    ...
    return Promise.all([
        promotions.weekly ? promoteBackup(promotions.weekly, 'weekly', weeklyDest) : null,
        promotions.monthly ? promoteBackup(promotions.monthly, 'monthly', monthlyDest) : null,
    ])
        .then(R.filter(x => !!x))
...
const removeExpiredLocalBackups = (num, infos) => {
}
