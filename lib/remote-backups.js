...
const getRemoteInfos = (s3, bucket, fileNameFormat, prefix) => {
    ...
    return Promise.all([
        getRemoteInfosSingle(s3, bucket, 'daily', fileNameFormat, frequencyPrefixes.daily),
        getRemoteInfosSingle(s3, bucket, 'weekly', fileNameFormat, frequencyPrefixes.weekly),
        getRemoteInfosSingle(s3, bucket, 'monthly', fileNameFormat, frequencyPrefixes.monthly),
    ])
        .then(R.flatten)
        .then(sortInfoNewestFirst)
}
...
const ensureRemoteBucket = (s3, bucket) => {
    return s3.createBucket({
        ...
        .catch(err => {
            if(err.code === 'BucketAlreadyExists') return null
            else throw err
        })
        .then(() => bucket)
}
